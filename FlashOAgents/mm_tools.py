...
from .mm_tools_utils import MarkdownConverter, FileConversionException, UnsupportedFormatException
...
This tool handles the following file extensions: [".txt", ".csv", ".md", ".log", ".html", ".pdb", ".xlsx", ".xls", ".pdf", ".docx", ".ppt", ".pptx"], and all other types of text files. IT DOES NOT HANDLE IMAGES."""
    ...
    def extract_excel_data(self, file_path, max_rows=30):
        ...
        try:
            ...
            if not result:
                return "Error: Spreadsheet appears to be empty."
    ...
    def forward_initial_exam_mode(self, file_path, question):
        try:
            ...
            if file_path.endswith(".xls"):
                fallback_text = self.extract_excel_data(file_path)
                result = type('Result', (object,), {'title': Path(file_path).name, 'text_content': fallback_text})
            else:
                raise
    ...
    def forward(self, file_path, question: Optional[str] = None) -> str:
        ...
        elif file_path.endswith((".txt", ".csv", ".log")):
            # Direct read for plain text files to preserve exact formatting including spaces
            try:
...
def get_single_file_description(file_path: str, question: str, visual_inspection_tool, document_inspection_tool, audio_inspection_tool):
    ...
    elif file_extension in ["pdf", "xls", "xlsx", "docx", "doc", "xml", "ppt", "pptx", "txt", "csv"]:
