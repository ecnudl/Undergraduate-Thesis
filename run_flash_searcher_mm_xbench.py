...
def process_item(item, model_config, summary_interval, prompts_type, max_steps, memory_type_str=None, enable_memory_evolution=True, judge_model=None, shared_memory_provider=None):
...
def main(args):
    ...
    judge_api_base = os.environ.get("JUDGE_API_BASE") or os.environ.get("OPENAI_API_BASE", "https://api-vip.codex-for.me/v1")
    ...
    if args.shared_memory_provider and args.memory_provider:
        if effective_concurrency != 1:
            raise ValueError("--shared_memory_provider requires --concurrency 1")
        shared_memory_provider = load_memory_provider(args.memory_provider, model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--judge_model', type=str, default=os.getenv('DEFAULT_JUDGE_MODEL', 'gpt-5'), help='Judge model id')
    ...
    parser.add_argument('--shared_memory_provider', action='store_true',
                        help='Reuse one memory provider across the full run (requires concurrency=1)')
